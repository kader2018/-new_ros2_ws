...
class AsterWalkNode(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(0.75, self.timer_callback)  # 50 Hz
    ...
    def radians_to_servo(self, rad):
        deg = math.degrees(rad)       # rad → degrés
        deg = max(-90, min(90, deg))  # Clamp logique
        pwm_deg = 90 + deg  # Centré à 90°
        pwm_deg = max(0, min(180, pwm_deg))  # Clamp hard
        print(f"[DEBUG] Radian: {rad:.3f} → Servo: {pwm_deg}°")
        return int(pwm_deg)
    ...
    def send_to_arduino(self, angles):
        ...
        try:
            ...
            msg = ";".join(values) + "\n"
            self.get_logger().warn(f"Degres: {values}")
    ...
    def timer_callback(self):
        ...
        positions[2] = -elbow                # bras_droit_rot_joint 
        positions[3] = 0.0         # bras_droit_coud_joint

        positions[4] = arm_amp        # bras_gauche_de_ar_joint   
        positions[5] = 0.0              # bras_gauche_h_b_joint
        positions[6] = elbow            # bras_gauche_rot_joint    
        positions[7] = 0.0            # bras_gauche_coude_joint
        ...
        for i in range(16):
            self.get_logger().warn(f"Valeur {positions[i] } : {i}")

      
        # Publier les angles
        self.publish_joint_positions(positions)
        self.send_to_arduino(positions)
        self.get_logger().warn(f"trame: {positions}")
        # RViz marker
        pt = Point()
        pt.x = x
        pt.y = 0.0
